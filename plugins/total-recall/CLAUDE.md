...
`install.sh` (plugin root) is the one-shot, state-aware setup script: profile prompt (a. minimal — no optional deps / b. complete — hybrid vector search, the DEFAULT; embeddings come from a local in-process HuggingFace MiniLM — no external service or daemon) → vault dirs → MCP registration → index build → optional standalone hook wiring / org vault / vector search → verify. Run `./install.sh --help` for flags (`-y`/`--yes`, `--default`/`--complete`/`--vector` (alias group), `--no-vector`, `--standalone`, `--statusline`, `--gemini`, `--copilot`, `--org-repo`, `--allowed-email-domain`, `--plugin-root`, `-h`/`--help`). User-facing install docs live in `INSTALL.md` (incl. Windows/Git Bash notes) — keep it in sync with install.sh flags. Its `--standalone` step embeds the canonical hooks JSON inline — keep that block in sync with `hooks/hooks.json` if hook commands, timeouts, or the build→load ordering change.
...
- 8.5: the three SessionStart hook manifests use a **per-client timeout field name** — `hooks.json` (Claude Code) and `hooks.gemini.json` (Gemini CLI) use `timeout` (ms), while `hooks.copilot.json` (GitHub Copilot CLI) uses `timeoutSec` (seconds). The field name is dictated by each client's hook schema, not a typo — do NOT "normalize" them to a single name or Copilot's hooks will reject the manifest. When adding a new SessionStart/SessionEnd/PreCompact hook, mirror the existing per-manifest field for that client. `install.sh` embeds the canonical hooks JSON inline for `--standalone` and must stay in sync with all three.
